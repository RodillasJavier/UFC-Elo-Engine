"""
UFC Elo engine — legacy fixed-K variant (superseded by engine.py).

Uses a flat K=40 for all outcomes and does not account for finish method.
Reads data/raw/ufcfights10_26_24.csv and writes
data/output/current_fighters_elo.csv.

Kept for reference; prefer engine.py for current rankings.
"""
...
def update_elo(winner_elo: float, loser_elo: float, k_factor: float) -> tuple[float, float]:
    ...
    Only handles win/loss; draws use k_factor/2 at the call site.
    """
...
#find the fighter with the lowest elo
