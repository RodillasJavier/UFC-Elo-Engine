...
UFC Elo engine — dynamic K-factor variant.

Reads data/raw/ufcfights.csv, processes every fight in chronological order,
and writes two output CSVs:
...
K-factor: 46 for KO/submission finishes, 40 for decisions.
Every fighter starts at Elo 1000.
...
def get_k_factor(method: str, base_k: float = 40) -> float:
  """Return K=46 for KO or submission finishes, base_k for all other methods."""
...
def update_elo(winner_elo: float, loser_elo: float, k_factor: float, result: str = "win") -> tuple[float, float]:
  """
  Apply one Elo update and return (new_winner_elo, new_loser_elo).

  result='draw' splits the expected score at 0.5; anything else (nc) leaves
  ratings unchanged.
  """
  ...
  if result == "win":
    ...
    # No change for 'no contest' (nc) or any other non-standard result
...
for index, row in ufcfights.iterrows():
  ...
  # Update the DataFrame with the starting Elo ratings
...
def get_fighter_info(fighter_name: str, elo_ratings: dict, ufcfights: pd.DataFrame, initial_elo: float = 1000):
  """Print and return a fighter's current Elo and full match history DataFrame.

  Returns a DataFrame of matches (if the fighter exists), or an explanatory string
  if they have no recorded fights.
  """
  ...
  fighter_matches = ufcfights[
    (ufcfights['fighter_1'] == fighter_name) |
    (ufcfights['fighter_2'] == fighter_name)
  ]

  # Return Elo rating and match history
...
all_fighters_df.to_csv(os.path.join(ROOT_DIR, "data/output/k_factor_adjust_current.csv"), index=False)
